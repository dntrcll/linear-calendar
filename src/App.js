...
const APP_META = { name: "Meridian", version: "3.5.0-Enterprise" };
...
// ==========================================
// 2. THEME ENGINE (Dark/Light/System)
// ==========================================
...
const CSS_INJECT = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; outline: none; -webkit-font-smoothing: antialiased; }
  ...
  .slide-in-right { animation: slideRight 0.3s var(--ease) forwards; }
  ...
  .shimmer::after { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(90deg, transparent, rgba(255,255,255,0.1), transparent); transform: translateX(-100%); animation: shimmer 2s infinite; }
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: theme.bg, color: theme.text }}>
      ...
      <aside style={{ 
        ...
        {/* Search & Mini Cal (Full Sidebar Only) */}
        {!settings.collapsed && (
          <div className="fade-in" style={{ flex: 1, display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={{ position: "relative" }}>
               <input 
                 ...
                 style={{ background: settings.darkMode ? theme.bg : theme.bg, color: theme.text, borderColor: theme.border }} 
               />
               ...
               <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4, textAlign: "center" }}>
                  ...
                  {(() => {
                     ...
                     while(d.getMonth() === currentDate.getMonth()) {
                       const c = new Date(d);
                       days.push(
                         <div key={d.getDate()} onClick={() => setCurrentDate(c)} 
                           style={{ 
                             fontSize: 11, padding: 4, borderRadius: 4, cursor: "pointer", 
                             background: isToday(c) ? theme.accent : "transparent", color: isToday(c) ? "#fff" : theme.text 
                           }}>
                           {d.getDate()}
                         </div>
                       );
            ...
            <div>
              ...
              {TAGS.map(t => (
                <div key={t.id} onClick={() => setSettings(s => ({...s, activeTags: s.activeTags.includes(t.id) ? s.activeTags.filter(x=>x!==t.id) : [...s.activeTags, t.id] }))} 
                  ...
                  <div style={{ width: 8, height: 8, borderRadius: "50%", background: t.border, boxShadow: `0 0 8px ${t.border}` }} />
        ...
        <div style={{ marginTop: "auto", borderTop: `1px solid ${theme.border}`, paddingTop: 16, display: "flex", flexDirection: settings.collapsed ? "column" : "row", gap: 8 }}>
           <button onClick={() => setSettings(s => ({...s, collapsed: !s.collapsed}))} className="btn-icon" data-tooltip="Toggle Sidebar" style={{ color: theme.textSecondary }}>
             {settings.collapsed ? '»' : '«'}
           </button>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", position: "relative" }}>
        ...
        <header style={{ height: CONFIG.HEADER_HEIGHT, borderBottom: `1px solid ${theme.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 32px", background: theme.bg }}>
          <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
            <h2 className="brand" style={{ fontSize: 24, fontWeight: 600, minWidth: 220 }}>
            ...
            <div style={{ display: "flex", background: theme.sidebar, padding: 4, borderRadius: 10, border: `1px solid ${theme.border}` }}>
              {['day', 'week', 'year'].map(m => (
                <button key={m} onClick={() => setViewMode(m)} style={{ 
                  padding: "6px 16px", borderRadius: 8, border: "none", fontSize: 13, fontWeight: 600, textTransform: "capitalize", cursor: "pointer", transition: "0.2s",
                  background: viewMode === m ? theme.bg : "transparent", color: viewMode === m ? theme.accent : theme.textSecondary, boxShadow: viewMode === m ? theme.eventShadow : "none" 
                }}>{m}</button>
              ))}
            </div>
            <div style={{ width: 36, height: 36, borderRadius: "50%", background: theme.accent, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14 }}>
            ...
            <div style={{ display: "flex", minHeight: "100%" }}>
              ...
              <div style={{ width: 60, flexShrink: 0, borderRight: `1px solid ${theme.border}`, background: theme.bg }}>
                {Array.from({length: 24}).map((_, h) => (
                  <div key={h} style={{ height: 60 * CONFIG.PIXELS_PER_MINUTE, position: "relative" }}>
                    <span style={{ position: "absolute", top: -6, right: 8, fontSize: 11, color: theme.textSecondary, fontWeight: 600, fontFamily: "var(--font-mono)" }}>
              ...
              <div style={{ flex: 1, display: "flex" }}>
                {(viewMode === 'day' ? [currentDate] : (() => {
                  ...
                  // Filter and Layout Events for this Column
                  ...
                  return (
                    <div key={colIndex} style={{ flex: 1, borderRight: `1px solid ${theme.border}`, position: "relative", background: isT ? (settings.darkMode ? "#131B2D" : "#F8FAFC") : "transparent" }} className={isPast ? "past-blur" : ""}>
                      ...
                      {/* Live Time Indicator */}
                      {isT && <div style={{ position: "absolute", top: (now.getHours() * 60 + now.getMinutes()) * CONFIG.PIXELS_PER_MINUTE, left: 0, right: 0, height: 2, background: theme.timeIndicator, zIndex: 10, pointerEvents: "none" }}>
                        <div className={settings.darkMode ? "pulse-active" : "pulse-active"} style={{ position: "absolute", left: -4, top: -4, width: 10, height: 10, borderRadius: "50%", background: theme.timeIndicator }} />
                      </div>}
                      ...
                      <div style={{ flex: 1, display: "flex", gap: 2 }}>
                        {Array.from({length: CONFIG.YEAR_COLUMNS}).map((_, col) => {
                          ...
                          return <div key={col} onClick={() => { setCurrentDate(date); setViewMode('day'); }}
                            ...
                            color: isT ? "#fff" : hasEv ? (settings.darkMode ? "#60A5FA" : "#0369A1") : theme.text, fontWeight: isT?700:400,
                            border: isT ? `1px solid ${theme.accent}` : "none" }}>{num}</div>
                        })}
                      </div>
                    </div>
                  );
        ...
        <div className="fade-in" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)" }} onClick={() => setShowSettings(false)}>
          <div style={{ width: 380, background: theme.card, padding: 24, borderRadius: 20, boxShadow: theme.shadow, border: `1px solid ${theme.border}` }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 24 }}>
              ...
              <button onClick={() => setShowSettings(false)} className="btn-icon" style={{color:theme.text}}>✕</button>
            </div>
      ...
      <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 200, display: "flex", flexDirection: "column", gap: 10 }}>
        {notifications.map(n => (
          <div key={n.id} className="fade-in" style={{ padding: "12px 24px", background: n.type==='error'?"#EF4444":n.type==='success'?"#10B981":theme.card, color: n.type==='info'?theme.text:"#FFF", borderRadius: 12, fontSize: 13, fontWeight: 600, boxShadow: "0 10px 40px -10px rgba(0,0,0,0.2)", border: n.type==='info'?`1px solid ${theme.border}`:'none', display: "flex", alignItems: "center", gap: 8 }}>
...
function AuthScreen({ onLogin, theme }) {
  return (
    <div style={{height:"100vh", background: "#0B0E11", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", position: "relative", overflow: "hidden"}}>
      ...
      <h1 className="brand" style={{fontSize: 72, color: "#fff", marginBottom: 24, letterSpacing: "-3px", zIndex: 1}}>Meridian.</h1>
    ...
    <div style={{ width: 440, background: theme.card, padding: 32, borderRadius: 24, boxShadow: theme.shadow }} onClick={e => e.stopPropagation()}>
      ...
      <div style={{display: "flex", flexDirection: "column", gap: 16}}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <div style={{flex: 1}}>
             <label style={{fontSize:11, fontWeight:700, color:theme.textSecondary, marginBottom:4, display:"block"}}>START</label>
             <input type="time" value={sTime} onChange={e => setSTime(e.target.value)} className="input-base" style={{ background: theme.bg, color: theme.text, borderColor: theme.border }} />
          </div>
          <div style={{flex: 1}}>
             <label style={{fontSize:11, fontWeight:700, color:theme.textSecondary, marginBottom:4, display:"block"}}>END</label>
             <input type="time" value={eTime} onChange={e => setETime(e.target.value)} className="input-base" style={{ background: theme.bg, color: theme.text, borderColor: theme.border }} />
          </div>
        </div>

        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {TAGS.map(t => <button key={t.id} onClick={() => setCat(t.id)} style={{ padding: "6px 12px", borderRadius: 20, border: cat === t.id ? `2px solid ${t.border}` : `1px solid ${theme.border}`, background: cat === t.id ? t.bg : "transparent", color: cat === t.id ? t.text : theme.text, fontSize: 12, fontWeight: 600, cursor: "pointer", transition: "0.2s" }}>{t.name}</button>)}
        </div>
